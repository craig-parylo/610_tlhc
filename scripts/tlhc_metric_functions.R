...
## based on cancer data --------------------------------------------------------

#' Get a cancer group lookup (icd10 -> cancer group)
#' 
#' load cancer grouping map table (maps icd-10 codes to broad anatomical site)
#' based on https://digital.nhs.uk/ndrs/data/data-sets/cosd/cosd-user-guide-v10/appendix
#' 
#' @return Tibble
get_df_icd_group <- function() {
  ...
  return(
    vroom::vroom(
      ...
      mutate(
        # remove the dot from icd
        icd_10 = str_remove_all(icd_10, '\\.'),
        
        # make a 3-character icd reference
        icd10_3c = str_sub(icd_10, 1, 3),
        
        # Convert 'other' and codes with multiple groups to 'Unknown' in keeping with TUMOUR_GROUP
        tumour_site_group_3c = case_when(
          icd10_3c %in% c('C76', 'C77', 'C78', 'C79', 'C80', 'C74', 'C75', 'C97') ~ 'Unknown',
          .default = tumour_site_group
        )
      ) |>
      # Add lookups for 'D' codes - which don't appear in this list but are found in rr_tumour TUMOUR_GROUP
      add_row(icd10_3c = 'D09', tumour_site_group_3c = 'Urological') |>
      add_row(icd10_3c = 'D41', tumour_site_group_3c = 'Urological') |>
      add_row(icd10_3c = 'D32', tumour_site_group_3c = 'Brain/Central Nervous System') |>
      add_row(icd10_3c = 'D33', tumour_site_group_3c = 'Brain/Central Nervous System') |>
      add_row(icd10_3c = 'D35', tumour_site_group_3c = 'Brain/Central Nervous System') |>
      add_row(icd10_3c = 'D42', tumour_site_group_3c = 'Brain/Central Nervous System') |>
      add_row(icd10_3c = 'D43', tumour_site_group_3c = 'Brain/Central Nervous System') |>
      add_row(icd10_3c = 'D44', tumour_site_group_3c = 'Brain/Central Nervous System') |>
      add_row(icd10_3c = 'D06', tumour_site_group_3c = 'Gynaecological') |>
      add_row(icd10_3c = 'D29', tumour_site_group_3c = 'Urological') |>
      add_row(icd10_3c = 'D48', tumour_site_group_3c = 'Unknown')
  )
}

#' Group diagnoses
#' 
#' Use the icd-10 to cancer group lookup to group cancers
#' 
#' @param df Tibble of tumour data (either cancer registry or rr tumour) to group cancers
#' @param df_icd_group Tibble of lookup values from icd10 code to tumour group
#' @param type Character description of the tibble passed in df - either 'registry' or 'tumour'
#' @return Tibble
group_cancers <- function(df, df_icd_group, type = 'registry') {
  
  # get a lookup between icd10 and cancer group
  icd_group_lu <- df_icd_group |> 
    filter(!is.na(icd_10)) |> 
    select(icd_10, tumour_site_group) |> 
    unique()
  
  # get a 3-char code lookup for icd10 to cancer group
  icd_group_lu_3c <- df_icd_group |> 
    select(icd10_3c, tumour_site_group_3c) |> 
    unique()
  
  # decide if 'registry' or 'tumour' table
  if (type == 'registry') {
    
    # this is the registry table - requires lookups from full-code and 3-code icd
    return(
      df |>
        # add in codes full matched
        left_join(
          y = icd_group_lu |>
            select(
        ...
        select(-tumour_site_group_1, -tumour_site_group_2)
    )
    
  } else {
    
    # this is a tumour table - only requires 3-code icd
    return(
      df |>
        # add in codes full matched
        left_join(
          y = icd_group_lu_3c |>
            select(TUMOUR_SITE = icd10_3c, tumour_site_group = tumour_site_group_3c) |>
            unique(),
          by = 'TUMOUR_SITE'
        )
    )
...
#' Identify tumours in cancer registry
#' 
#' Looks up the corresponding cancer registry TUMOUR_ID (where applicable)
#' 
#' @param df Tibble of RR tumours
#' @param df_events Tibble as a subset of RR pathway containing just dates of diagnoses
#' @return Tibble
identify_registry_tumourid <- function(df, df_events) {
  
  # step 1 link RR tumours with cancer registry tumours
  df_lookup <- df |> 
    # small subset of data
    select(TUMOUR_AVP_ID, ParticipantID, DIAGNOSISDATE) |>
    unique() |> 
    # add in lookup values
    left_join(
      y = df_events,
      by = 'ParticipantID',
      relationship = 'many-to-many' # there will be multiple diagnosis records per rr tumour
    ) |> 
    # work out difference between dates
    mutate(
      days_difference = date_count_between(
        start = DIAGNOSISDATE |> as.Date(),
        end = EVENT_DATE |> as.Date(),
        precision = 'day'
      ) |> 
        abs()
    ) |> 
...
#' Identify tumours in cancer registry
#' 
#' Looks up the corresponding cancer registry TUMOUR_ID (where applicable) using
#' the NCARDS approach (join on Participant ID and Tumour type and limit to 90 days difference)
#' 
#' @param df Tibble of RR tumours
#' @param df_events Tibble as a subset of RR pathway containing just dates of diagnoses
#' @return Tibble
identify_registry_tumourid_v2 <- function(df, df_reg) {
  
  # step 1 link RR tumours with cancer registry tumours
  df_lookup <- df |> 
    # small subset of data
    select(TUMOUR_AVP_ID, ParticipantID, DIAGNOSISDATE, tumour_site_group) |>
    unique() |> 
    # add in lookup values
    left_join(
      y = df_reg |> select(ParticipantID, tumour_site_group, TUMOUR_ID, DIAGNOSISDATEBEST),
      by = c('ParticipantID', 'tumour_site_group'),
      relationship = 'many-to-many' # there will be multiple diagnosis records per rr tumour
    ) |> 
    # work out difference between dates
    mutate(
      days_difference = date_count_between(
        start = DIAGNOSISDATE |> as.Date(),
        end = DIAGNOSISDATEBEST |> as.Date(),
        precision = 'day'
      ) |> 
        abs()
    ) |> 
    # remove outliers
    filter(days_difference <= 90) |> 
    # keep best match per tumour
    group_by(TUMOUR_AVP_ID) |> 
    slice_min(n = 1, order_by = days_difference) |> 
    ungroup() |>
...
#' Get a dataframe of consolidated cancer records
#' 
#' @return Tibble
get_df_cancer <- function() {
  
  p('df cancer outcomes')
  
  # load the data if not already loaded
  if(!exists('df_canc_registry')){df_canc_registry <- load_df_cancer_reg()}
  if(!exists('df_canc_rr_tumour')){df_canc_rr_tumour <- load_df_cancer_tumour()}
  if(!exists('df_icd_group')){df_icd_group <- get_df_icd_group()}
  
  ## get a list of diagnosis events ---
  df_events <- df_canc_rr_pathway |> 
    # subset fields for efficiency
    select(ParticipantID, EVENT_DATE, EVENT_TYPE, TUMOUR_ID) |> 
    # limit to diagnosis type events
    filter(EVENT_TYPE %in% c(6, 100)) |>
    # remove any duplication
    unique()
  
  ## tumour staging group ---
  df_canc_registry <- df_canc_registry |> 
    # add some groupings
    group_cancers(
      df_icd_group = df_icd_group,
      type = 'registry'
    ) |> 
    mutate(stage_group = str_sub(STAGE_BEST, 1, 1))
  
  df_canc_rr_tumour <- df_canc_rr_tumour |> 
    # add some groupings
    group_cancers(
      df_icd_group = df_icd_group,
      type = 'tumour'
    ) |> 
    mutate(stage_group = str_sub(STAGE, 1, 1)) |> 
    # identify tumours present in cancer registry
...
get_df_cancer_v2 <- function() {
  ...
  df_canc_rr_tumour <- df_canc_rr_tumour |> 
    ...
    mutate(stage_group = str_sub(STAGE, 1, 1)) |> 
    # identify tumours present in cancer registry
    identify_registry_tumourid_v2(df_reg = df_canc_registry)
