...
#' Get data for Sankey
#' 
#' NB, relies on having pre-loaded metrics data
#'
...
# get aggregate flows of data when working with record-level
...
# get an up-to-date snapshot of data
df_sankey <- get_data_for_sankey()

# pre-aggregate the sankey df
df_sankey_preagg <- df_sankey |>
...
saveRDS(object = df, file = here('outputs', 'df_sankey.Rds'))
saveRDS(object = df_sankey_preagg, file = here('outputs', 'df_sankey_preagg.Rds'))

# plot a sankey diagram for the data
get_sankey_for_data(df = df_sankey_preagg)
